...
<section class="emerging-methods">
  <div class="methods-intro">
    <div>
      <span class="eyebrow">Research frontier</span>
      <h2>Emerging approaches to watch</h2>
    </div>
...
<section class="method-route">
  ...
  <div class="source-sensor-summary">
    ...
    <div><strong>Sensors</strong><span>Magnetometers, comagnetometers, and vapor-cell spin sensors</span><span>Solid-state spin sensors</span><span>Torsion pendula</span><span>Particle beams</span></div>
  </div>
...
<section class="method-catalogue">
  <div class="methods-intro">
    <div>
      <span class="eyebrow">Data-linked catalogue</span>
      <h2>Experiments represented in the database</h2>
    </div>
  ...
  <article class="method-family" id="dedicated-beam" data-method-family="dedicated-beam">
    <header><span>D</span><div><h3>Beam and particle sensors</h3><p>Neutron-spin rotation and other dedicated particle-based source–sensor measurements.</p></div><strong class="family-count">…</strong></header>
    <div class="method-record-list"><p class="method-loading">Reading curated metadata…</p></div>
  </article>
...
<aside class="methods-scope-note">
  <strong>Scope of this directory</strong>
  <p>The catalogue lists records that can be assigned to an experimental method from their curated metadata. Combined limits and purely astrophysical constraints remain available in the Limits Explorer, but are not treated here as laboratory methods. Method definitions and notation follow the <a href="https://doi.org/10.1103/RevModPhys.97.025005">RMP review</a>.</p>
  <a class="button button-primary" href="{{ '/explorer-beta.html' | relative_url }}">Open Limits Explorer →</a>
</aside>
...
(() => {
  ...
  const labels = {
    optically_polarized_vapor: "Optically polarized vapor",
    atomic_magnetometer_array: "Atomic-magnetometer array",
    atomic_magnetometer: "Atomic magnetometer",
    diffusion_optical_pumping_magnetometer: "Diffusive atomic magnetometer",
    self_compensating_comagnetometer: "Self-compensating comagnetometer",
    quantum_spin_amplifier: "Quantum spin amplifier",
    spin_based_amplifier: "Spin-based amplifier",
    noble_gas_spin_amplifier: "Noble-gas spin amplifier",
    single_nv_center: "Single NV center",
    single_electron_spin_quantum_sensor: "Single-electron-spin sensor",
    ensemble_nv_diamond_magnetometer: "Ensemble NV-diamond magnetometer",
    spin_mechanical_quantum_sensor: "Spin-mechanical quantum sensor",
    spin_polarized_torsion_pendulum: "Spin-polarized torsion pendulum",
    earth_source_spin_experiment: "Earth-source spin experiment",
    geoelectron_source_spin_experiment: "Geoelectron-source spin experiment",
    ...
    atomic_parity_violation_interpretation: "Atomic parity-violation interpretation",
    atomic_spectroscopy_reanalysis: "Atomic spectroscopy",
    molecular_spectroscopy: "Molecular spectroscopy",
    molecular_ruler_spectroscopy: "Molecular-ruler spectroscopy",
    atomic_hyperfine_spectroscopy: "Atomic hyperfine spectroscopy",
    helium_fine_structure_spectroscopy: "Helium fine-structure spectroscopy",
    antiprotonic_helium_spectroscopy: "Antiprotonic-helium spectroscopy",
    hydrogen_spectroscopy: "Hydrogen spectroscopy",
    molecular_beam_magnetic_resonance: "Molecular-beam magnetic resonance",
    trapped_ion_spin_sensor: "Trapped-ion spin sensor",
    spin_exchange_collision_analysis: "Spin-exchange collision analysis",
  ...
  const familyFor = (record, reference) => {
    ...
    if (method.category === "dedicated_source_sensor" || /dedicated/.test(method.category || "")) {
      if (/polarized_helium_spin_relaxation|vapor_cell_spin_relaxation/.test(value)) return "dedicated-magnetic";
  ...
  Promise.all(files.map(file => fetch(file.path)
    ...
    .then(payloads => {
      ...
      document.querySelectorAll("[data-method-family]").forEach(section => {
        const family = section.dataset.methodFamily;
        const entries = Object.values(grouped)
          .filter(entry => entry.family === family)
          .sort((a, b) => Number(b.year || 0) - Number(a.year || 0) || a.citation.localeCompare(b.citation));
        section.querySelector(".family-count").textContent = `${entries.length} methods / papers`;
        const list = section.querySelector(".method-record-list");
        if (!entries.length) {
          list.innerHTML = '<p class="method-loading">No classified records yet.</p>';
          return;
        }
        list.innerHTML = entries.map(entry => {
          ...
          return `<div class="method-record">
            <div class="method-record-kind">${escapeHTML(labels[entry.technique] || entry.technique.replaceAll("_", " "))}</div>
            <div class="method-record-copy">
